...
//Nuova implementazione salvataggi da menu retroarch
...
void update_prefs_retrocfg(struct uae_prefs * prefs, bool loadlha)
{
 ...
 struct retro_variable varmulw = {0};
  ...
  var.key = "uae4arm_fastmem";
   ...
   var.key = "uae4arm_input_autofire_framecnt";
   var.value = NULL;
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
      if (strcmp(var.value, "Disabled") == 0)
	  {
		  prefs->input_autofire_framecnt=0;
	  }
      else
	  {
		//Implementazione dell'autoload dello state sullo slot 12, serve   
        prefs->input_autofire_framecnt = atoi(var.value) ;
		  
      }
   }
 
   var.key = "uae4arm_input_autofire_framecnt_joy2";
   var.value = NULL;
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
      if (strcmp(var.value, "Disabled") == 0)
	  {
		  prefs->input_autofire_framecnt_joy2=0;
	  }
      else
	  {
	      prefs->input_autofire_framecnt_joy2 = atoi(var.value) ;
