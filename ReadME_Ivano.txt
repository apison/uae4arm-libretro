* Added various option to menu libretro
* Fixed mouse button 2
* Joystick 2 Enabled, is now possible to play in 2
* Added Save State, they can be made or recalled on new page 3 Of Virtual Keyboard
* Added function to swap disk directly from Virtual Keyboard, Disk must be renamed as (Disk 1),(Disk x)....
* Sostituita la tastiera con una migliore che supporta anche le trasparanze
* Aggiunto supporto ai Whdload
* Aggiunto Mode Auto sella selezione del modello.
* Corretti problemi abilitazione secondo Joystick
* Aggiungia opzione fastcopper anche se sembra non fare niente
* Abilitati SaveState anche per il whdload
* Fissato save da whdload quando impostato come files
* Aggiunto nelle impostazioni multiplier  per modificare la risoluzione a piacimento, è possibile modificare sia la larghezza che l'altezza.
* Fissato cambio delle impostazioni grafiche in retroarch che non richiedono piu' il riavvio
* Aggiunto Autofire per Joy1 e Joy2 bisognare sciegliere nel menu ogni quanti frame si attiverà o disabilitarlo.
* Aggiornata interfaccia retroarch implementata spedizione messaggi a retroarch nel save/load state e nel cambio dischetti.
* Added delete Savestate into Page 2 of keyboard
* Aggiunti Save State gestiti da retroarch tramite serializzazione, purtroppo il save_sate pur iniziando al punto in cui lo si è richiesto, viene eseguito dopo circa 40 secondi e ne impiega piu' o meno altrettanti per essere eseguito, ho preferito quindi tenere anche i savestate richiamabili da keyboard in quanto vengono eseguiti in maniera istantanea.
* Aggiunto nelle opzioni retroarch la possibilità di salvare e caricare gli state (non serialize), selezionare lo slot dove salvare o da cui caricare, al resume dell'emulazione verrà fatto, questo si è reso necessario a causa dell'estrema lentezza dei salvataggi ufficiali (metodi serialize), sono stati aggiunti nelle opzioni retroarch perchè piu' immediatamente raggiungibili rispetto al richiamo da tastiera che rimane comunque impleentata, inoltre usando il menu retroarch l'emulazione va in pausa permettendo il salvataggio con piu' calma.  
* Fissato audio che non funzionava correttamente nella ultima versione di Retroarch 